  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      <header className="bg-teal-950 text-white py-4 px-6 flex justify-between items-center shadow-md">
        ...
        <h1 className="text-lg font-bold tracking-wide">My Purchase Orders</h1>
          ...
          <div className="space-y-6">
            {orders.map((order) => (
              <div key={order.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                <div className="bg-gray-55/70 px-6 py-4 border-b border-gray-200/80 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                  <div>
                    <span className="text-xs text-gray-400 font-mono block">ORDER ID: {order.id}</span>
                ...
                <div className="bg-gray-50/50 px-6 py-4 border-t border-gray-150 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                  <div className="max-w-md">
                    {order.adminNotes && (
                      <div className="text-xs">
                        <span className="font-bold text-gray-700 block">Admin/Order Notes:</span>
                        <p className="text-gray-500 mt-0.5">{order.adminNotes}</p>
                      </div>
                    )}
                  </div>
                  <div className="text-right">
                    <span className="text-xs text-gray-400 block font-medium">Grand Total (incl. GST)</span>
